...
namespace lab618 {
	...
	private:
		struct block {
			// Ìàññèâ äàííûõ áëîêà
			T* pdata = nullptr;
			// Àäðåñ ñëåäóþùåãî áëîêà
			block *pnext = nullptr;
			// Ïåðâàÿ ñâîáîäíàÿ ÿ÷åéêà
			int firstFreeIndex = 0;
			// ×èñëî çàïîëíåííûõ ÿ÷ååê
			int usedCount = 0;
	...
	public:
		...
		// Ïîëó÷èòü àäðåñ íîâîãî ýëåìåíòà èç ìåíåäæåðà
		T* newObject() {
		...
		// Îñâîáîäèòü ýëåìåíò â ìåíåäæåðå
		...
		// Î÷èñòêà äàííûõ, çàâèñèò îò m_isDeleteElementsOnDestruct
		void clear() {
			if (nullptr == m_pBlocks) {
				return;
			}
	...
	private:
		...
		// Ñîçäàòü íîâûé áëîê äàííûõ. ïðèìåíÿåòñÿ â newObject
		block* newBlock() {
		...
		// Îñâîáîäèòü ïàìÿòü áëîêà äàííûõ. Ïðèìåíÿåòñÿ â clear
		...
		// Ðàçìåð áëîêà
		int m_blkSize = 0;
		// Íà÷àëî ñïèñêà áëîêîâ
		block* m_pBlocks = nullptr;
		// Òåêóùèé áëîê
		block* m_pCurrentBlk = nullptr;
		// Óäàëÿòü ëè ýëåìåíòû ïðè îñâîáîæäåíèè
